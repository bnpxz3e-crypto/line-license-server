// License server for LINE OA Auto Calculator V8
// Node.js 18+
...
app.get('/',(req,res)=>res.json({ok:true,name:'LINE OA Auto Calculator License Server',version:'V8'}));
app.get('/admin',(req,res)=>res.sendFile(path.join(__dirname,'admin.html')));

app.post('/api/check',(req,res)=>{
 const key=String(req.body?.key||'').trim(); const db=load(); const x=db[key];
 const active=!!x && !x.revoked;
 res.json({ok:true,active,key:active?key:undefined,expiresAt:null});
});
...
app.post('/api/admin/licenses',auth,(req,res)=>{
 ...
 const db=load();
 if(db[key]) return res.status(409).json({ok:false,error:'key already exists'});
 db[key]={revoked:false,createdAt:new Date().toISOString(),note:String(req.body?.note||'')};
 save(db);
